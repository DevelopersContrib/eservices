 ...
 **/
  ...
  **/
  ...
  $(".poll_form_widget").submit(function(e){
    e.preventDefault();
    var f=$(this);
    var action=f.attr("action");
    var r=document.referrer;
    $.ajax({
      ...
      success: function (r) {                              
        f.parent("div").html(r);
        animate_progress();
		$('.view-results-trigger').off('click').on('click',function(){
  ...
  **/
  ...
  $('.view-results-trigger').on('click',function(){
	$('.view-results').each(function(){
		$(this).trigger('click');
	});
  });
...
function showViewResult(){
	if($('.tabs').length == $('.view-results').length){
		$('.view-results-trigger').parent().show();
	}
}
